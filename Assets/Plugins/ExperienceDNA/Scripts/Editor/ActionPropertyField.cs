...
[CustomPropertyDrawer(typeof(ActionSelector))]

public class ActionPropertyField : PropertyDrawer
{
    private static BaseDataService _localDataController;
    private string[] _empty = new string[]{"-"};
    [JsonIgnore]
    private static HashSet<ActionConfiguration> _data;
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (_data == null)
        {
            ...
            _data = _localDataController.DataStructure?.AllActions;
        }
        ...
        if (_data?.Count > 0)
        {
            int index = Mathf.Max(_data.ToList().IndexOf(_data.ToList().FirstOrDefault(r => r.ConfigId == id)), -1);
            if (index == -1) {
            ...
            int selectedIndex = EditorGUI.Popup(position, property.displayName, index, _data.Select(r => r.Name).ToArray());
            var confId = _data.ToList()[selectedIndex].ConfigId;
            property.FindPropertyRelative("_value").intValue = confId;
        }
        else
        {
            property.FindPropertyRelative("_value").intValue = EditorGUI.IntField(position, $"Unable to fetch actions ({property.displayName})", id);
        }
        EditorGUI.EndChangeCheck();
    }
}