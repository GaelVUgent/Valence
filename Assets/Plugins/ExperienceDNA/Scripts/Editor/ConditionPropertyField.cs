...
[CustomPropertyDrawer(typeof(ConditionSelector))]
[CanEditMultipleObjects]
public class ConditionPropertyField : PropertyDrawer
{
    private static BaseDataService _localDataController;
    private string[] _empty = new string[] { "-" };
    [JsonIgnore]
    private static HashSet<Condition> _data;
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (_data == null)
        {
            ...
            _data = _localDataController.DataStructure?.AllConditions;
            if (_data != null) { 
                _data.Add(
                    new Condition()
                    {
        ...
        {
            property.FindPropertyRelative("_value").intValue = EditorGUI.IntField(position, $"Unable to fetch conditions ({property.displayName})", id);
        }
        EditorGUI.EndChangeCheck();
    }
}